<template>
    <div>
        <div class="rounded bg-gray-800 p-4 text-2xl w-40 mx-auto" :class="{'bg-yellow-800':warn.ispast,'bg-red-800':alarm.ispast}">
            {{timeleft}}
        </div>
        <button @click="start" class="m-2 bg-green-800" :disabled="running && !paused" :class="{'opacity-50':running&&!paused}">start</button>
        <button @click="stop" class="m-2 bg-red-800" :disabled="!running || paused" :class="{'opacity-50':!running||paused}">stop</button>
        <button @click="reset" class="m-2 bg-blue-800" :disabled="!running" :class="{'opacity-50':!running}">reset</button>
        <div class="w-fit mx-auto text-right">
            <label class="p-2 block">
                Alarm Time (minutes)
                <input type="number" v-model="alarm.minutes" class="w-14" :disabled="running" :class="{'opacity-50':running}" />
            </label>
            <label class="p-2 block">
                Warn Time (minutes)
                <input type="number" v-model="warn.minutes" class="w-14" :disabled="running" :class="{'opacity-50':running}" />
            </label>
        </div>
    </div>
</template>
<script lang="ts">
import { defineComponent } from 'vue'

export default defineComponent({
    data() {
        return {
            alarm: {
                ispast: false,
                minutes: 4,
                remainingms: 0
            },
            warn: {
                ispast: false,
                minutes: 2,
                remainingms: 0
            },
            timerinterrupt: false
        }
    },
    computed: {
        timeleft():string {
            if (this.alarm.remainingms == 0)
                return '--:--';
            const ss = Math.floor(this.alarm.remainingms / 1000);
            const mm = Math.floor(ss / 60);
            return `${mm.toString().padStart(2, '0')}:${(ss - (mm * 60)).toString().padStart(2, '0')}`;
        },
        running():boolean {
            return this.alarm.remainingms != 0;
        },
        paused():boolean {
            return this.timerinterrupt;
        }
    },
    methods: {
        start() {
            if (this.alarm.remainingms == 0) { // init times if not set yet, otherwise use existing times
                this.alarm.remainingms = (this.alarm.minutes * 60 * 1000);
                this.warn.remainingms = (this.warn.minutes * 60 * 1000);
            }
            this.timerinterrupt = false;
            this.step();
        },
        stop() {
            this.timerinterrupt = true;
        },
        reset() {
            this.timerinterrupt = true;
            this.alarm.ispast = false;
            this.alarm.remainingms = 0;
            this.warn.ispast = false;
            this.warn.remainingms = 0;
        },
        step() {
            if (!this.timerinterrupt) {
                const interval = 1000; // 1000ms => 1s
                this.alarm.remainingms = this.alarm.remainingms - interval;
                if (this.warn.remainingms > 0) { // only if a warning has been set
                    this.warn.remainingms = this.warn.remainingms - interval;
                }
                if (this.alarm.remainingms <= 0 && !this.alarm.ispast) {
                    this.alarm.ispast = true;
                    this.playchime();
                    this.stop();
                } else if (this.warn.remainingms <= 0 && !this.warn.ispast) {
                    this.warn.ispast = true;
                    this.playchime();
                } else {
                    setTimeout(this.step, interval); // take into account drift
                }
            }
        },
        playchime() {
            const audio = new Audio('data:audio/ogg;base64,');
            audio.play();
        }
    }
})
</script>
<style scoped>
.w-fit { width:fit-content; }
</style>